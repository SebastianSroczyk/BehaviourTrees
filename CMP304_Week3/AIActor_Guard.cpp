...
void AIActor_Guard::Update(float _dt)
{
	...
	if (guardState == GuardState::Moving)
	{
		...
		if (movePoint >= 1)
		{
			...
			EditBBValueInt("ReachedDesintation", 1);
			hasMoveTarget = false;
			guardState = GuardState::Stationary;
			...
			if (isRaisingAlarm)
			{
				EditBBValueInt("HasRaisedAlarm", 1);
...
void AIActor_Guard::MoveToPoint(sf::Vector2f _targetPoint, bool _sprint)
{
	...
	EditBBValueInt("ReachedDesintation", 0);
}
...
ActionStatus AIActor_Guard::GetAlarmPath()
{
	...
	hasMoveTarget = true;
	isRaisingAlarm = true;
	EditBBValueInt("IsRaisingAlarm", 1);
	return ActionStatus::ACTION_SUCCESS;
}
...
ActionStatus AIActor_Guard::GetInvestigatePath()
{
	...
	AIMath_Global::Vector2f _noisePoint = GetBBValueVec2f("PlayerHeard");

	EditBBValueInt("CanHearPlayer", 0); // reset so that once finished investigating, don't try to investigate the same point
...
ActionStatus AIActor_Guard::Patrol()
{
	...
	if (GetBBValueInt("ReachedDesintation") == 1) {
		return ActionStatus::ACTION_SUCCESS;
	}
...
ActionStatus AIActor_Guard::RaiseAlarm()
{
	...
	if (GetBBValueInt("HasRaisedAlarm") == 1) {
		
		EditBBValueInt("IsRaisingAlarm", 0);
		return ActionStatus::ACTION_SUCCESS;
	}
...
ActionStatus AIActor_Guard::Chase()
{
	...
	if (chaseCounter <= 0)
	{
		...
		AIMath_Global::Vector2f _playerPoint = GetBBValueVec2f("PlayerSeen");
...
{
	if (GetBBValueInt("CanSeePlayer") == 1)
		return true;
	else
		return false;
}
...
{
	if (GetBBValueInt("CanHearPlayer") == 1)
		return true;
	else
		return false;
}
...
void AIActor_Guard::UpdateCanSeePlayer(float _x, float _y)
{
	...
	EditBBValueInt("CanSeePlayer", 1);
	EditBBValueVec2f("PlayerSeen", AIMath_Global::Vector2f(_x, _y));
...
void AIActor_Guard::UpdateCannotSeePlayer()
{
	if (sightCounter <= 0)
	{
		...
		EditBBValueInt("CanSeePlayer", 0);
		DeleteBBValueVec2f("PlayerSeen");
...
{
	EditBBValueInt("CanHearPlayer", 1);
	EditBBValueVec2f("PlayerHeard", AIMath_Global::Vector2f(_x, _y));

}

void AIActor_Guard::UpdateCannotHearPlayer()
{
	EditBBValueInt("CanHearPlayer", 0);
	DeleteBBValueVec2f("PlayerHeard");
